...
namespace AutoPrice.Crawlers.Ferroalloy.MolybdenumAlloy
{
     public class MolybdenumIron : ICrawler
    {
        private const int MarketId = 567;
        ...
        public void Run()
        {
            ...
            try
            {
                ...
                var silicon = driver.FindElement(By.LinkText("钼系"));
                ...
                var ferrosilicons = driver.FindElements(By.LinkText(date + "钼铁价格汇总"));
                if (ferrosilicons.Count > 0)
                {
                    ...
                    var titleTxt = firstTds[2].Text.Trim();
                    ...
                    {
                        if(trs[1].Text.Trim().Contains("陕西"))//先确定表格里面有没有陕西的价格 如果没有 后面就取河北或者河南的价格
                        {
                            isHasSX = true;
                        }
                    }
                    ...
                    for (int i = 1; i < trs.Count; i++)
                    {
                        ...
                        var tags = new string[2];//把价格列之前的所有数据用|隔开组成一个字符串
                        for (int j = 0; j < 2; j++)
                        {
                            tags[j] = tds[j].Text;
                        }
                        ...
                        var priceStr = tds[2].Text.Trim();//找到价格列的值
                        ...
                        var tdDelta = tds[3].Text.Trim();//涨跌
