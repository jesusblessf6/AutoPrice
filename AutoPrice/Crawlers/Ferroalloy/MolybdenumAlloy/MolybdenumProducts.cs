...
{
     public class MolybdenumProducts : ICrawler
    {
        ...
        public void Run()
        {
            ...
            try
            {
                ...
                var ferrosilicons = driver.FindElements(By.LinkText(date + "钼制品价格汇总"));
                if (ferrosilicons.Count > 0)
                {
                    ...
                    for (int i = 1; i < trs.Count; i++)
                    {
                        ...
                        token += tds[3].Text + "|";//跳过第三列单位列 把第4列加上
                        p.Token = token;

                        var unit = tds[2].Text.Trim();//钼制品单位列
                        p.PriceUnit = unit;
